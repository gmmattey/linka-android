...
fun OrbitSymbol(
    ...
    val ringScale by animateFloatAsState(
        targetValue = if (state == OrbitState.Success || state == OrbitState.Warning || state == OrbitState.Critical) 1f else 0.9f,
...
@Deprecated("Use OrbitSymbol", ReplaceWith("OrbitSymbol(state, modifier, size)"))
@Composable
fun LinkaPulseSymbol(
    state: OrbitState,
    modifier: Modifier = Modifier,
    size: Dp = 120.dp,
) = OrbitSymbol(state, modifier, size)
