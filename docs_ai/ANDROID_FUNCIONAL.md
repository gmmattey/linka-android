...
**Última atualização:** 2026-07-05 (v0.23.0 — versionCode 56; LinkaPulseScreen→SignallQPulseScreen; FibraScreen→FibraModemScreen; abas/overlays reconciliados com AppShell.kt)
...
> Fonte de verdade: git log + código real.
...
Verifica-se usando `io.signallq.app.FeatureFlags` em qualquer tela ou lógica. As features pós-MVP estão mapeadas em `FeatureFlags.kt` e controladas no `app/build.gradle.kts`.
...
├── DiagnosticoScreen → LLMChatScreen [FEATURE_DIAGNOSTICO_CHAT — ativo em release]
├── AjustesScreen → FibraModemScreen [FEATURE_FIBRA_SCREEN — ativo]
...
> Nota de navegação: em `AppShell.kt`, `tabScreenNames = listOf("home", "speedtest", "sinal_wifi", "historico", "ajustes")`. A NavBar reflete labels: Início / Velocidade / Sinal / Histórico / Ajustes. Dispositivos, Fibra e Diagnóstico/IA são overlays empilhados, não abas — não existe aba "Mais".
...
| `FibraModemScreen` | `ArrowBack` no navigationIcon — sempre visível |
...
## 6. Telas Secundárias (Fluxos Sobrepostos / Overlays)
...
**Botões:** "Conversar com IA" (exibido quando `FEATURE_DIAGNOSTICO_CHAT` ativo — ativo em release), "Testar Upload Novamente", "Ir para o início", "Testar novamente".
...
**Flag de controle:** `FEATURE_DIAGNOSTICO_CHAT` — **ativa em release** (promovida para release no build.gradle.kts). Visível para o usuário final.
...
### 6.7 FibraModemScreen — Modem GPON
...
**Distinção com `LLMChatScreen`:** `SignallQScreen` é o chat diagnóstico autônomo com chips contextuais e pipeline de engines (`SignallQOrchestrator`). `LLMChatScreen` é o chat livre controlado por `FEATURE_DIAGNOSTICO_CHAT`. Ambos coexistem.

### 6.9 SignallQPulseScreen

**Composable:** `SignallQPulseScreen.kt` (ex-`LinkaPulseScreen`)

**Flag de controle:** `FEATURE_LINKPULSE_ATIVO` — **inativo em release**. Visível apenas em builds debug.

Dashboard de monitoramento contínuo. Exibe o símbolo SignallQ Pulse animado com status do monitoramento passivo.
...
3. ResultadoVelocidadeScreen: resultado completo
   ...
   └── [opcional] "Conversar com IA" → LLMChatScreen [FEATURE_DIAGNOSTICO_CHAT — ativo em release]
```
...
> **Disponibilidade:** `FEATURE_DIAGNOSTICO_CHAT` — **ativo em release**. Este fluxo está visível para o usuário final em produção.
...
| v0.15.0 | Rebranding Veloo → SignallQ, identidade visual, OrbitScreen→SignallQScreen, Orbit*→SignallQ* componentes (caminho físico legado `io/veloo/app`) |
...
| v0.16.0+ | Renomeação de package/namespace/applicationId para `io.signallq.app` (2026-06-28, antes de publicação); caminho físico dos fontes mantido em `io/veloo/app` |
| v0.23.0 | versionCode 56 (build atual, 2026-07-05); LinkaPulseScreen→SignallQPulseScreen; ajustes de Home e detalhes de conexão |
