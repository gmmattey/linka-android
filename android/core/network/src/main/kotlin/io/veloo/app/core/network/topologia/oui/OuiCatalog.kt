...
import io.signallq.app.core.network.contracts.oui.NivelValidacaoOui
import io.signallq.app.core.network.contracts.oui.OuiEntry
import io.signallq.app.core.network.contracts.topologia.PapelTopologia

/**
 * Catálogo OUI único do app — Fase 1 do plano de unificação de topologia (issue #975/#978).
 *
 * Unifica o que antes eram duas listas paralelas com curadoria independente:
 * - `OuiDatabase` (feature/devices): ~130 prefixos com fabricante, sem papel de topologia.
 * - `MeshOuiDatabase` (coreNetwork): `MESH_NO_OUIS`/`GATEWAY_ISP_OUIS`, com papel de topologia,
 *   sem fabricante.
 *
 * O conflito de curadoria da Intelbras (`C46E1F`/`6C5AB0`, cadastrado nas duas listas de papel
 * ao mesmo tempo) vira aqui um único [OuiEntry] com `papeisPossiveis = {ROTEADOR, NO_MESH}` — o
...
object OuiCatalog {
    ...
    fun lookup(mac: String?): OuiEntry? {
        val prefixo = normalizarPrefixo(mac) ?: return null
    ...
    // Sistemas mesh conhecidos — nós secundários. Ex-MESH_NO_OUIS de MeshOuiDatabase.
    private val meshNoOuis: Set<String> = setOf(
    ...
    // Gateways/roteadores ISP Brasil — provavelmente roteador principal. Ex-GATEWAY_ISP_OUIS.
    private val gatewayIspOuis: Set<String> = setOf(
    ...
    // Nome do fabricante pra quem só existia em meshNoOuis (sem entrada em fabricantesPorPrefixo).
    private val meshNoFabricantes: Map<String, String> = buildMap {
        listOf("F4F5D8", "1AC487", "48D705", "94EB2C", "A4977A").forEach { put(it, "Google") }
        listOf("50C7BF", "B09575", "C006C3", "E8DE27", "1027F5", "A42BB0", "74DA88").forEach { put(it, "TP-Link") }
        listOf("F4F5E8", "98F1F2", "8C8590", "7C571C").forEach { put(it, "Eero (Amazon)") }
        listOf("9C3DCF", "B0B98A", "28C68E", "A040A0").forEach { put(it, "Netgear") }
        listOf("C46E1F", "6C5AB0").forEach { put(it, "Intelbras") }
        listOf("788A20", "24A43C", "E063DA", "802AA8", "74ACB9", "F09FC2").forEach { put(it, "Ubiquiti") }
        listOf("0CE76F", "8843E1", "34BD3B").forEach { put(it, "Cisco/Meraki") }
        listOf("D8C7C8", "6C722D", "24DEC6").forEach { put(it, "Aruba/HP") }
    }

    // Nome do fabricante pra quem só existia em gatewayIspOuis (sem entrada em fabricantesPorPrefixo).
    private val gatewayIspFabricantes: Map<String, String> = buildMap {
        listOf("C46E1F", "B4A9FC", "6C5AB0").forEach { put(it, "Intelbras") }
        listOf("2C26C5", "00E0FC", "ACF7F3", "14ADED", "88B111").forEach { put(it, "ZTE") }
        listOf("286ED4", "9C74B0", "D440F0", "4C8BEF", "A0808F").forEach { put(it, "Huawei") }
        listOf("001BC5", "A4CAA0", "3C8CF8").forEach { put(it, "Nokia/Alcatel-Lucent") }
        listOf("549F13", "D86CE9", "B4A512").forEach { put(it, "Sagemcom") }
        listOf("10BF48", "30D3C7", "E49665").forEach { put(it, "Technicolor/Thomson") }
        listOf("000AE7", "B0487A").forEach { put(it, "Mitrastar") }
        listOf("14D64D", "1C7EE5", "28107B", "84C9B2").forEach { put(it, "D-Link") }
        listOf("50FA84", "10BEF5", "A0F3C1", "04D9F5").forEach { put(it, "TP-Link") }
    }

    // Base curada de prefixos OUI (primeiros 6 hex do MAC, sem dois pontos, uppercase).
    // Fonte: IEEE OUI registry — ~130 prefixos de maior ocorrência em redes domésticas
    // brasileiras. Ex-`OuiDatabase.db` (feature/devices).
    private val fabricantesPorPrefixo: Map<String, String> = mapOf(
        // Apple
    ...
    private val entries: Map<String, OuiEntry> = buildCatalog()

    private fun buildCatalog(): Map<String, OuiEntry> {
        val todosPrefixos = fabricantesPorPrefixo.keys + meshNoOuis + gatewayIspOuis
        return todosPrefixos.associateWith { prefixo ->
            val papeisPossiveis = buildSet {
                if (prefixo in meshNoOuis) add(PapelTopologia.NO_MESH)
                if (prefixo in gatewayIspOuis) add(PapelTopologia.ROTEADOR)
            }
            val fabricante = fabricantesPorPrefixo[prefixo]
                ?: meshNoFabricantes[prefixo]
                ?: gatewayIspFabricantes[prefixo]
                ?: "Desconhecido"
            OuiEntry(
                prefixo = prefixo,
                fabricante = fabricante,
                papeisPossiveis = papeisPossiveis,
                nivelValidacao = NivelValidacaoOui.CURADO_MANUALMENTE,
                especificidade = if (papeisPossiveis.isNotEmpty()) {
                    EspecificidadeOui.LINHA_PRODUTO_ESPECIFICA
                } else {
                    EspecificidadeOui.FABRICANTE_GENERICO
                },
            )
        }
    }
