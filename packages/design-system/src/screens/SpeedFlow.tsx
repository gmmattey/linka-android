...
export interface SpeedFlowProps {
  /** Called when the user navigates away (e.g. "Ir para o início" or "Conversar com a IA") */
  onNavigate?: (dest: string) => void;
}

/** Full speed-test flow: idle start button → animated gauge → results with verdicts. */
export function SpeedFlow({ onNavigate }: SpeedFlowProps) {
  const [phase, setPhase] = useState<'idle' | 'running' | 'result'>('idle');
  const [mode, setMode] = useState('Completo');

  if (phase === 'idle') {
    return <SpeedIdle mode={mode} setMode={setMode} onStart={() => setPhase('running')} />;
  }
  if (phase === 'running') {
    return <SpeedRunning onDone={() => setPhase('result')} />;
  }
  return (
    <Resultado
      onAgain={() => setPhase('idle')}
      onHome={() => onNavigate?.('home')}
      onSignallq={() => onNavigate?.('signallq')}
    />
  );
}
    ...
    >
      ...
      >
        <button
          ...
          style={{
            ...
            color: '#fff',
            font: `700 30px/1 ${LK.font}`,
            boxShadow: `0 0 0 10px ${hexA(LK.accent, 0.18)}`,
          }}
        >
          Iniciar
        </button>
          ...
          return (
            <button
              ...
              style={{
                ...
                background: on ? LK.bgCard : 'transparent',
                color: on ? LK.textPrimary : LK.textSecondary,
      ...
      >
        ...
        <div style={{ display: 'flex' }}>
          {(
            [
              ['Download', '486', LK.success],
              ['Upload', '212', LK.accent],
              ['Latência', '12', LK.success],
            ] as const
          ).map(([l, v, c]) => (
            <div key={l} style={{ flex: 1 }}>
              <div style={{ font: `400 10px/1 ${LK.font}`, color: LK.textTertiary, marginBottom: 4 }}>
                {l}
              </div>
              <div style={{ font: `700 20px/1 ${LK.font}`, color: c }}>
                {v}
                <span style={{ font: `400 10px/1 ${LK.font}`, color: LK.textSecondary, marginLeft: 2 }}>
                  {l === 'Latência' ? 'ms' : 'Mbps'}
                </span>
              </div>
            </div>
...
function SpeedRunning({ onDone }: { onDone: () => void }) {
  const phases = [
    { key: 'LATÊNCIA', tone: '#60A5FA', unit: 'ms', target: 12 },
    ...
    >
      <div style={{ position: 'relative', width: 240, height: 240 }}>
        ...
        >
          ...
          <div style={{ font: `600 12px/1 ${LK.font}`, color: cur.tone, marginTop: 10, letterSpacing: '.5px' }}>
            {cur.key}
          </div>
        </div>
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: ok ? hexA(LK.success, 0.12) : active ? hexA(LK.accent, 0.12) : LK.bgSecondary,
                color: ok ? LK.success : active ? LK.accent : LK.textTertiary,
              }}
            >
              {ok && <Icon name="check" size={13} color={LK.success} />}
              {k}
            </div>
          );
        })}
...
function Resultado({
  ...
  const Metric = ({
    ...
    <div style={{ background: LK.bgSecondary, borderRadius: LK.rCard, padding: 16 }}>
      ...
      <div style={{ font: `700 24px/1 ${LK.font}`, color }}>
        {value}
        <span style={{ font: `400 11px/1 ${LK.font}`, color: LK.textSecondary, marginLeft: 3 }}>{unit}</span>
      </div>
    </div>
  );

  const verdict = (icon: string, label: string, word: string, tone: string) => (
