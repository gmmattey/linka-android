...
**Diagnóstico:**
```bash
gh run list --repo 7ALabs/linka-android --branch <branch> --json status,conclusion,workflowName
```
Se o run mais recente tiver `conclusion: action_required`, aprove antes de confiar em qualquer
status:
```bash
gh api -X POST repos/7ALabs/linka-android/actions/runs/<run-id>/approve
```
...
repos/7ALabs/linka-android/pulls/<N>/update-branch`.
