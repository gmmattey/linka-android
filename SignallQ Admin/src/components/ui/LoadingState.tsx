...
export const LoadingState: React.FC<LoadingStateProps> = ({
  ...
  return (
    <div
      id={id || "loading-state"}
      className="flex flex-col items-center justify-center py-12 px-4 rounded-[var(--radius-card)] border border-dashed border-zinc-800/80 bg-zinc-900/10"
    >
      ...
      <div className="w-full max-w-md mt-6 space-y-2.5 animate-pulse">
        {Array.from({ length: rows }).map((_, idx) => (
          <div key={idx} className="h-4 sq-skeleton rounded-[var(--radius-button)] w-full flex items-center px-4 gap-4">
